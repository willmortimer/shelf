...
pub async fn serve(socket_path: impl AsRef<Path>, store: MemoryStore) -> Result<(), DaemonError> {
    #[cfg(not(unix))]
    {
        let _ = (socket_path.as_ref(), store);
        Err(DaemonError::UnsupportedOs)
    }
    #[cfg(unix)]
    {
        serve_unix(socket_path.as_ref(), store).await
    }
}

#[cfg(unix)]
async fn serve_unix(socket_path: &Path, store: MemoryStore) -> Result<(), DaemonError> {
...
fn dispatch(req: IpcRequest, store: &mut MemoryStore) -> IpcResponse {
    match req {
        IpcRequest::Put { bytes, kind, name } => match store.put(bytes, kind, name) {
            Ok((id, created)) => IpcResponse::Put { id, created },
            Err(err) => store_error(err),
        },
        IpcRequest::Ls => IpcResponse::Ls { items: store.ls() },
        ...
        IpcRequest::Get { target } => match store.get(&target) {
...
#[cfg(unix)]
fn store_error(err: crate::store::StoreError) -> IpcResponse {
    match err {
        crate::store::StoreError::NotFound => IpcResponse::Error {
            code: IpcErrorCode::NotFound,
            message: "object not found".into(),
        },
        crate::store::StoreError::Protocol(err) => IpcResponse::Error {
